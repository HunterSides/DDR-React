...
const SignUp = () => {
  ...
  React.useEffect(() => {
    const mnemonic = getMnemonic() // getsMnemonic from localstorage
    getDashAccount(mnemonic) // creates new Client connection/sets account = client.getWalletAccount()
      .then((account) => {
        console.log('account information', account) // returns (address/balance/mnemonic)
        setAccountInfo(account)
        const {
          mnemonic,
          balance: { confirmed: balance },
        } = account //updates account balance variable based on a mnemonic being funded
        if (balance === 0) {
