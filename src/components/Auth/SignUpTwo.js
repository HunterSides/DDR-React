...
import { getDashAccount, getMnemonic, init } from 'utils'
...
const SignUpTwo = () => {
  ...
  /* 
  1) init replaces getMnemonic with user.init
  2) replaces getDashAccount with user.wallet.read()
 */
  React.useEffect(() => {
    const init = async () => {
      if (isInitialized) {
      ...
      const wallet = await user.wallet.read()
      //setAccountInfo(wallet)
      if (wallet.balance.confirmed > 0) {
        console.log('tDash found, heres your account info', wallet)
        //let balanceConfirmed = wallet.balance.confirmed
        ...
        console.log('please fund wallet to continue', wallet.address)
  ...
  /* React.useEffect(() => {
    ...
    getDashAccount(mnemonic)
      .then((account) => {
        setAccountInfo(account) //account.getidentities
        const {
          mnemonic,
